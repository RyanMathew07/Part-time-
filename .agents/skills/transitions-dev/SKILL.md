...
| **Tilt card** | 3D pointer tilt with cursor-tracked glare overlay. | (inline in globals.css) |
| **Plus morph** | Button surface morphs into a menu panel with cross-fade. | (inline in globals.css) |
| **Accordion** | Grid-rows height animation with chevron flip (no JS measuring). | (inline in globals.css) |
...
- **3D hover interaction / depth / perspective** → tilt card.
- **Small FAB that expands into a panel of choices** → plus morph.
- **Collapsible section / disclosure widget / expand/collapse content** → accordion.
...
The value vocabulary behind the transitions. All tokens are defined as CSS custom properties in `:root` in [globals.css](file:///d:/Documents/Antigravity/Resellie/src/app/globals.css). Every pattern references these instead of hardcoded values.

**Durations** (`--duration-*`)

| Variable | Value | Name | Usage |
| --- | --- | --- | --- |
| `--duration-stagger` | 40ms | Stagger | per-item stagger offset |
| `--duration-micro` | 80ms | Micro | tooltip delay, shake segment, large stagger |
| `--duration-quick` | 150ms | Quick | modal close, dropdown close, text swap, tooltip appear |
| `--duration-fast` | 250ms | Fast | icon swap, dropdown open, modal open, tabs sliding, page slide |
| `--duration-medium` | 350ms | Medium | panel close, toast close |
| `--duration-slow` | 400ms | Slow | panel open, skeleton content reveal, input clear |
| `--duration-very-slow` | 500ms | Very slow | emphasis moments, badge appear, text reveal, success check |

**Easings** (`--ease-*`)

| Variable | Value | Name | Usage |
| --- | --- | --- | --- |
| `--ease-smooth-out` | cubic-bezier(0.22, 1, 0.36, 1) | Smooth ease out | modal / dropdown / panel open + close, page slide, resize, position change |
| `--ease-in-out` | ease-in-out | Ease in out | icon swap, text swap, text reveal, skeleton reveal |
| `--ease-out` | ease-out | Ease out | tooltip open / close |
| `--ease-linear` | linear | Linear | shimmer, skeleton pulse, spinner |
| `--ease-bounce` | cubic-bezier(0.34, 1.36, 0.64, 1) | Bouncy overshoot | badge pop open |
| `--ease-bounce-strong` | cubic-bezier(0.34, 3.85, 0.64, 1) | Strong bouncy overshoot | bouncy hover-out (avatar return) |

**Distances** (`--distance-*`)

| Variable | Value | Name | Usage |
| --- | --- | --- | --- |
| `--distance-micro` | 4px | Micro | text swap |
| `--distance-small` | 6px | Small | error shake (small segment) |
| `--distance-base` | 8px | Base | badge diagonal reveal, page slide, error shake (large segment) |
| `--distance-medium` | 12px | Medium | text reveal |
| `--distance-large` | 30px | Large | check badge appear |

**Scales** (`--scale-*`)

| Variable | Value | Name | Usage |
| --- | --- | --- | --- |
| `--scale-large` | 0.96 | Large | modal open / close |
| `--scale-medium` | 0.97 | Medium | dropdown open |
| `--scale-small` | 0.98 | Small | tooltip open |
| `--scale-tiny` | 0.99 | Tiny | dropdown close |

**Blur** (`--blur-*`)

| Variable | Value | Name | Usage |
| --- | --- | --- | --- |
| `--blur-small` | 2px | Small | panel reveal, icon swap, text swap, skeleton reveal, number pop-in |
| `--blur-medium` | 3px | Medium | page slide, text reveal |
| `--blur-large` | 8px | Large | success check open |
